...
| 2026-08-30 | **NODE PROPOSAL INTEGRATION COMPLETE（11-7）**：`assembly.rs` 实现 `handle_proposal`（`decode_proposal_ref` 64B → 既有 `ConsensusEvent::SetProposal` → `transition` 不修改 → `apply_result` 路由）+ `NodeError::ProposalDecode`；`handle_envelope` 支持 `ConsensusProposal`（原 unsupported 测试改用 `ConsensusQc`，QC 仍 DEFERRED）。nova-node 11 tests（valid proposal Applied step→Prevote / 拒非法 64B payload / 非 Propose 阶段 Ignored{ContextMismatch}）；四项 Gate 全 PASS；Node 不验证 proposer authority（A11 DEFERRED）/ 不执行 verify_qc / 不新增 event variant / 不修改 transition | 用户授权 STEP 11-7 Node Proposal Integration → commit `ce0d055` |
| 2026-08-30 | **NODE PROPOSAL INTEGRATION FINAL FREEZE（11-7）**：STEP 11-7 全链（serialization + Node Proposal Integration）彻底封版。仅文档变更记录更新；**不修改实现代码/不改变协议语义**。QC ingestion / A11 保持 DEFERRED；transition / ConsensusEvent 未触碰；external.rs NOT CREATED | 用户裁决 → Node Proposal Integration FINAL FREEZE（独立 documentation commit） |
